...
class Project(QObject):
    ...
    def __init__(self, parent=None):
        ...
        self._pilares_union_cache = None
    ...
    def invalidate_geometry_cache(self) -> None:
        """Clear pillar union cache and all beam polygon caches."""
        self._pilares_union_cache = None
        for v in self.vigas:
            v.invalidate_cache()
    ...
    def add_pilar(self, pilar: Pilar) -> None:
        """Add a new Pilar to the project."""
    ...
    def remove_pilar(self, pilar_id: str, record: bool = True) -> None:
        ...
        if not pilar:
            return
        if record:
            self._push_undo(('pilar_removed', pilar))
    ...
    def get_pilares_union(self):
        """Returns the unary_union of all pillar footprints. Cached for performance."""
        if self._pilares_union_cache is not None:
            return self._pilares_union_cache
            
        try:
            from shapely.ops import unary_union
            from shapely.geometry import Polygon
            polys = [Polygon(p.footprint_2d()) for p in self.pilares]
            self._pilares_union_cache = unary_union(polys) if polys else None
        except ImportError:
            self._pilares_union_cache = None
